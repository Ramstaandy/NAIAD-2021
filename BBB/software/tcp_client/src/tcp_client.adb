...
--        ada.Text_IO.Put_Line(radix.Practical_Radix("vsf")'img);
--
--        xPingMsg.Set_Field("target","NUL");
      ...
      while iTargetPort = -1 loop
         ...
         begin
            ...
            sTargetIp := (others => ' ');
            --              ada.Text_IO.open(File => ConnectionFile, Name => "serverip.txt", Mode => ada.Text_IO.In_File);
         ...
         exception
            when E : others =>
               --                 ada.Text_IO.Put_Line(ada.Exceptions
...
--                    Send(xPingMsg,bSucMsg);
   ...
   begin
      if cx.Is_Connected then
         ...
         -- Moves the json object to a string and sends it
         declare
...
set_buffer(cx, 1500);
   ...
   procedure Set_IP_And_Port (sIP : string; iPort : string) is
      xCurTime : Ada.Real_Time.Time := Ada.Real_Time.Clock;
      xInterval :  constant ada.Real_Time.Time_Span :=  ada.Real_Time.Milliseconds(50);
   begin
      ...
      if sIP'Length <= sInputIP'Length then
         ...
         iInputPort := radix.Practical_Radix(iPort);
         bValidIP := true;
         sSenderName := iPort;
