...
--              Option => (Name    => GNAT.Sockets.Receive_Timeout, timeout =>cx.timeout));
   ...
   exception
      when E : others =>
         ada.Text_IO.Put_Line("Could not start the server (missing sudo?)");
         Ada.Text_IO.Put_Line("Exception: " & Exception_Name(E) & ": " & Exception_Message(E));
         loop
            delay(5.0);
         end loop;
...
--
--        ada.Text_IO.Put_Line("tic");
--        Check_Selector(Cx.Selector, Cx.NSet, Cx.WSet, Cx.Status, cx.Timeout);
--        ada.Text_IO.Put_Line("toc");
--
--
...
--
--     exception when E : others =>
--           Ada.Text_IO.Put_Line("Error in reading from TCP buffer");
--           Ada.Text_IO.Put_Line("Exception: " & Exception_Name(E) & ": " & Exception_Message(E));
--           raise NetWork_Error;
--
...
Ada.Text_IO.Put_Line("1");
      ...
      if cx.Connected then
         ...
         if cx.EnableTimeout then
            ...
            Check_Selector(Cx.Selector, Cx.RSet, Cx.WSet, Cx.Status, cx.timeout);
