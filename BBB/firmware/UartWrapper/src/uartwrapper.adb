...
package body UartWrapper is
   ...
   procedure Uart_tcflush (this : in out CUartHandler) is
      function tcflush (port : C.int; buffer_type : C.int) return C.int;
      pragma Import (Convention => C, Entity => tcflush, External_Name => "tcflush");
      function fflush (port : C.int) return C.int;
      pragma Import (Convention => C, Entity => fflush, External_Name => "fflush");
      use type C.int;
      R : C.int;
   begin
      R := tcflush (this.serialHandler, 3);
      pragma Assert (R = 0, "tcflush (x, 3) error.");
      R := fflush (this.serialHandler);
      pragma Assert (R = 0, "fflush (x) error.");
   end;
